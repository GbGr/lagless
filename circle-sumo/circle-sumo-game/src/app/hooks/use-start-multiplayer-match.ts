...
const SERVER_URL = _params.get('serverUrl') || import.meta.env.VITE_RELAY_URL || 'ws://localhost:3333';
const SCOPE = _params.get('scope') || 'circle-sumo';
...
export const useStartMultiplayerMatch = () => {
  ...
  const handleMatchFound = useCallback((data: MatchFoundData, playerId: string) => {
    ...
    // Store the match info on the provider for runner-provider to use
...
// Extend type to carry match info to runner-provider

import { ECSDeps, IFilterConstructor, IFilterInstance } from '@lagless/types';
  ...
  public updateEntityInAllFilters(entity: number, componentsMask: number): void {
    for (const filterInstance of this._filtersInstances.values()) {
      if (filterInstance.includeMask && filterInstance.excludeMask) {
        if (this.checkComponentsMaskInclusion(componentsMask, filterInstance.includeMask) && this.checkComponentsMaskExclusion(componentsMask, filterInstance.excludeMask)) {

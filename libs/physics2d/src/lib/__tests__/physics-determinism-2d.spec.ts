...
interface SimulationSetup {
  simulation: PhysicsSimulation2d;
...
function setupScene(worldManager: PhysicsWorldManager2d): void {
  // Create a ground plane
  const ground = worldManager.createFixedBody();
  ground.setTranslation({ x: 0, y: -1 }, false);
  worldManager.createCuboidCollider(50, 1, ground);

  // Create a falling circle
  const circle = worldManager.createDynamicBody();
  circle.setTranslation({ x: 0, y: 10 }, true);
  worldManager.createBallCollider(0.5, circle);

  // Create another circle offset
  const circle2 = worldManager.createDynamicBody();
  circle2.setTranslation({ x: 2, y: 15 }, true);
  worldManager.createBallCollider(0.3, circle2);
}

function createPhysicsSystem(worldManager: PhysicsWorldManager2d): IECSSystem {
  return { update: () => worldManager.step() };
}

describe('Physics2d Determinism', () => {
  const managers: PhysicsWorldManager2d[] = [];

    ...
    websocket: {
      ...
      message(ws: ServerWebSocket<WsData>, message: string | Buffer<ArrayBuffer>) {
        ...
        if (data.type === 'match') {
          ...
          const messageArrayBuffer: ArrayBuffer | null =
            message instanceof ArrayBuffer ? message : null;
...
function handleMatchmakingMessage(
  ...
  if (msg.type === 'join') {
    ...
    matchmaking.addPlayer(data.playerId, msg.scope, 1000, msg.metadata ?? {}, (notification) => {

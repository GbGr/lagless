...
const SERVER_URL = _params.get('serverUrl') || import.meta.env.VITE_RELAY_URL || 'ws://localhost:3336';
const SCOPE = _params.get('scope') || '2d-map-test';
...
export const useStartMultiplayerMatch = () => {
  ...
  const handleMatchFound = useCallback(
    (data: MatchFoundData, playerId: string) => {
      ...
      const inputProvider = new RelayInputProvider(data.playerSlot, ecsConfig, MapTestInputRegistry);

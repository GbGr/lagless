...
export const useStartMultiplayerMatch = () => {
  ...
  const handleMatchFound = useCallback(
    (data: MatchFoundData, playerId: string) => {
      const ecsConfig = new ECSConfig({ fps: 60, maxPlayers: 4 });
      const inputProvider = new RelayInputProvider(data.playerSlot, ecsConfig, RobloxLikeInputRegistry);
  ...
  return { state, queuePosition, error, startMatch, cancel };
};
